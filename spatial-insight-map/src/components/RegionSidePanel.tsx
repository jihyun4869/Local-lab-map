...
export const RegionSidePanel: React.FC<RegionSidePanelProps> = ({
  ...
  return (
    <aside className="w-[420px] bg-white h-full border-l border-gray-200 shadow-2xl z-20 flex flex-col shrink-0 font-sans animate-in slide-in-from-right duration-300">
      {/* Side Panel Header */}
        ...
        <div className="flex items-center space-x-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center space-x-2">
              <h2 className="text-base font-bold text-white tracking-tight truncate">{regionPref.name}</h2>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        ...
        <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-3.5 space-y-3">
          <div className="flex items-center justify-between pb-2 border-b border-blue-100">
            <div className="flex items-center space-x-2 text-blue-700 font-bold text-xs">
              <span className="w-2.5 h-2.5 rounded-full bg-blue-600"></span>
              <span>선호 요인</span>
            </div>
            <span className="text-[11px] bg-blue-100 text-blue-700 font-bold px-2.5 py-0.5 rounded-full">
          ...
          <div className="space-y-1.5 max-h-52 overflow-y-auto pr-1">
            {regionPref.prefItems.length === 0 ? (
              <p className="text-xs text-blue-500/80 py-3 text-center font-medium bg-white/40 rounded-xl border border-blue-100/50">
              ...
              regionPref.prefItems.map((item, idx) => (
                ...
                >
                  <label className="flex items-center space-x-2.5 text-xs font-medium cursor-pointer flex-1 select-none min-w-0">
                    <input
                      type="checkbox"
        ...
        <div className="bg-red-50/50 border border-red-100 rounded-2xl p-3.5 space-y-3">
          <div className="flex items-center justify-between pb-2 border-b border-red-100">
            ...
            <span className="text-[11px] bg-red-100 text-red-700 font-bold px-2.5 py-0.5 rounded-full">
          ...
          <div className="space-y-1.5 max-h-52 overflow-y-auto pr-1">
            {regionPref.disprefItems.length === 0 ? (
              <p className="text-xs text-red-500/80 py-3 text-center font-medium bg-white/40 rounded-xl border border-red-100/50">
      ...
      <div className="p-3.5 bg-gray-50 border-t border-gray-200 flex items-center justify-between shrink-0">
        <div className="flex items-center space-x-2.5 text-xs font-semibold text-gray-600">
        ...
        <button
          onClick={onClose}
          className="bg-slate-900 text-white font-bold text-xs py-2 px-5 rounded-xl hover:bg-slate-800 transition shadow-md cursor-pointer active:scale-95 flex items-center space-x-1.5"
        >
